...
export function ShareButton({ title, slug, variant = "icon" }: ShareButtonProps) {
  const [copied, setCopied] = useState(false);
  const { toast } = useToast();

  const baseUrl = "https://lightandtruth.com.ng";
    ...
    <Dialog>
      <DialogTrigger asChild>
        {variant === "button" ? (
          ...
          <button className="inline-flex items-center gap-1 text-muted-foreground hover:text-primary transition-colors text-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            {socials.map((social) => (
              <a
                ...
                className={`flex items-center gap-2 p-3 rounded-lg ${social.color} transition-colors`}
              >
                <social.icon />
